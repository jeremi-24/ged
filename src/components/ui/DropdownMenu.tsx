...
import { ChevronRightIcon, Plus, SettingsIcon } from "lucide-react";
...
type DropdownMenuProps = {
  items: { icon: React.ReactNode; name: string; href?: string; customStyle?: string }[];
  containerClassName?: string;
  itemClassName?: string;
};

export default function DropdownMenuplus({
  items,
  containerClassName,
  itemClassName,
}: DropdownMenuProps) {
  ...
  return (
    <nav
      className={cn(
        "max-w-[90px] w-full mx-auto space-y-2",
        containerClassName
      )}
      ref={scope}
    >
      ...
      >
        <div style={{ transformOrigin: "50% 55%" }}>
          <Plus size={34} className="text-neutral-400" id="menu-icon" />
        </div>
      </motion.button>
      <ul
        className={cn(
